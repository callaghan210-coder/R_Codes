# Set your working directory to the directory where you have saved your data files 
...
# Biologist worried about potential health effects of music  festivals.	
# Measured hygiene of 810 concert-goers over  the three days of a music festival.
...
# First, produce the histogram for the normally distributed data (normal) 
#and add a normal curve using the mean and standard deviation of the variable.

par(mfrow=c(2,3)) # Combine all six plots below in 1 plot

h <- hist(dlf$day1,probability=T, breaks = 30, main="Histogram of hygiene score on day 1",xlab="Hygiene score on day 1", ylab = "Density")
# add a normal curve using the mean and standard deviation of the variable.
...
h <- hist(dlf$day2,probability=T, breaks = 30, main="Histogram of hygiene score on day 2",xlab="Hygiene score on day 2", ylab = "Density")
...
h <- hist(dlf$day3,probability=T, breaks = 30, main="Histogram of hygiene score on day 2",xlab="Hygiene score on day 2", ylab = "Density")
...
# This graph plots the cumulative values we have in our data against the cumulative 
# probability of a particular distribution 
#(in this case we would specify a normal distribution). 
# If the data are normally distributed then the actual scores will have the 
# same distribution as the score we expect from a normal distribution, and you'll get a 
...
qqnorm(dlf$day1, pch = 1, frame = FALSE, main = "Normal Q-Q Plot Day 1")
qqline(dlf$day1, col = "steelblue", lwd = 2)

#Q-Q plot for day 2:

qqnorm(dlf$day2, pch = 1, frame = FALSE, main = "Normal Q-Q Plot Day 2")
qqline(dlf$day2, col = "steelblue", lwd = 2)

#Q-Q plot of the hygiene scores on day 3:
qqnorm(dlf$day3, pch = 1, frame = FALSE, main = "Normal Q-Q Plot Day 3")
qqline(dlf$day3, col = "steelblue", lwd = 2)
...
# Therefore, having inspected the distribution of hygiene scores visually, 
# we can move on to look at ways to quantify the shape of the distributions and to look for outliers. 
...
# In this function, we simply name our variable and by default 
#(i.e., if we simply name a variable and don't include the other commands)
# we'll get a whole host of statistics including some basic ones such as the number of cases 
# (because basic = TRUE by default) but not including statistics relating to the normal
# distribution (because norm = FALSE by default). 
# Specify basic = FALSE to get rid of these statistics, but in the current context 
# it is useful to override the default and specify norm = TRUE so that we get statistics 
# relating to the distribution of scores. Therefore, we could execute:
...
#Multiple variable - method 2 (subsetting and roundingn to 3 digits)
...
#This file contains data regarding students' performance on an R exam. 
#Four variables were measured: exam (first- year R exam scores as a percentage), 
#computer (measure of computer literacy as a percentage), lecture 
#(percentage of R lectures attended) and numeracy (a measure of numerical ability out of 15). 
# There is a variable called uni indicating whether the  student  attended  
# Sussex  University  or  Duncetown  University.  
...
h <- hist(rexam$exam,probability=T, breaks = 30, main="Histogram of Exam Anxiety: First Year Exam Scors",xlab="First Year Exam Score", ylab = "Density")
...
h <- hist(rexam$computer, probability=T, breaks = 30, main="Histogram of Exam Anxiety: computer Literacy",xlab="Computer Literacy", ylab = "Density")
...
# The exam scores are very interesting because this distribution is 
# quite clearly not normal; in fact, it looks suspiciously bimodal (there are two 
# peaks, indicative of two modes)- we come back to this later in the codes 

#Percentage of Lectures Attended
h <- hist(rexam$lectures, probability=T, breaks = 30, main="Histogram of Exam Anxiety: Percentage of Lectures Attended",xlab="Percentage of Lectures Attended", ylab = "Density")
...
# Numeracy
h <- hist(rexam$numeracy, probability=T, breaks = 30, main="Histogram of Exam Anxiety: Numeracy",xlab="Numeracy", ylab = "Density")
...
sussexData<-subset(rexam, rexam$uni=="Duncetown University")

h <- hist(dunceData$numeracy, probability=T, breaks = 10, main="Histogram of numeracy for Duncetown University students",xlab="Numeracy Score", ylab = "Density")
...
h <- hist(dunceData$exam, probability=T, breaks = 35, main="Histogram of first year exam score for Duncetown University students",xlab="First Year Exam Score", ylab = "Density")
...
h <- hist(sussexData$numeracy, probability=T, breaks = 10, main="Histogram of numeracy for Sussex University students",xlab="Numeracy Score", ylab = "Density")
...
h <- hist(sussexData$exam, probability=T, breaks = 35, main="Histogram of first year exam score for Sussex University students",xlab="First Year Exam Score", ylab = "Density")
...
# Note: the exam marks the distributions are both fairly normal. This seems odd because the overall 
# distribution was bimodal. However, it starts to make sense when you consider that for 
# Duncetown the distribution is centred on a mark of about 40%, but for Sussex the distribution 
#is centred on a mark of about 76%. This illustrates how important it is to look at 

...
# Example: aggregate data frame mtcars by cyl and vs, 
...
# You must specify the variables needed to uniquely identify 
# each measurement (ID and Time)

#Cast function
# Now that the data is in a melted form, it can be recast
#into any shape, using the cast() function.
...
newdata_mtcars_wt <- subset(mtcars, wt >= 3.610 | wt < 2.58, select=c(mpg, cyl, disp, wt))
 
print(newdata_mtcars_wt)


# EaIn the next example, we select cars with miles per gallon greater than 20.09 and 
#carburetors = 2 but we keep variables from horsepower to 1/4 mile time (hp, drat, wt and qsec).
...
mysample_mtcars <- mtcars[sample(1:nrow(mtcars), 15, replace=TRUE),]
mysample_mtcars
...
expenditures <- read.delim("E:/Projects/Session 3/expenditures.txt", sep="")
...
# Example: Continuing with the sub.exp dataframe which includes only the recent 5 years worth of
# expenditures, we can filter by Division:
sub.exp %>% filter(Division == 6)

# Filtering by multiple criteria within a single logical expression (starwars is an R dataset)
none_black<-filter(starwars, hair_color == "none" & eye_color == "black")
View(none_black)
print(none_black)
# Dplyr: group_by( ) function
# Objective: Group data by categorical variables

# Description: Often, observations are nested within groups or categories and our goals is to
#perform statistical analysis both at the observation level and also at the group level.
#The group_by() function allows us to create these categorical groupings.
# The group_by() function is a silent function. No observable manipulation of the data is
#performed after applying the function.
...
# Dplyr: summarise( ) function
# Previous slide: useful summaries. Comparison of summary statistics at multiple levels reveals
# important insights.
# This is where the group_by() function comes in. Let's group by Division and see how the
# different regions compared in by 2010 and 2011.
sub.exp %>%
    group_by(Division)%>%
...
# Description: Often, we desire to view observations in rank order for a particular variable(s). The
# arrange() function allows us to order data by variables in accending or descending order.

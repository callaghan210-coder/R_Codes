#  Online Academic Data Analysis Bootcamp Using
#             Open-Access Program R
#                    Base Plots in R
# Introduction: generic plot types in R

setwd("~/Documents/R ISADS/Essential Level/Session Four/Practicals")
rm(list = ls())
#Examples
...
# Sturges' rule is a simple and commonly used method to determine the number of breaks. It suggests that the number of breaks (k) should be approximately equal to 1 + log2(n), where n is the number of data points in the dataset.
...
row.names(mtcars), groups = grps, gcolor =
my_cols, color = my_cols[grps], cex = 0.6, pch
= 19, xlab = "mpg")
...
dotchart(VADeaths, cex = 0.6, main = "Death
Rates in Virginia - 1940")
...
barplot(x, names.arg = c("Grp1", "Grp2", "Grp3"))

# Change color
# Change border and fill color using one single color
barplot(x, col = "wheat3", border = "steelblue")
...
barplot(x, main = "Death Rates in
Virginia",  xlab = "Age", ylab = "Rate")
...
barplot(VADeaths, col = c("lightblue", "mistyrose", "lightcyan",
"lavender", "cornsilk"), legend = rownames(VADeaths), beside = TRUE)
...
legend("topleft", legend = rownames(VADeaths),
     fill = my_colors, box.lty = 0, cex = 0.9)
# dev.off()
# Notes:
...
pie(df$value, labels = df$group, radius = 0.8)

# Pie Charts: Change colors


pie(df$value, labels = df$group, radius = 1, col
= c("#999999", "#E69F00", "#56B4E9"))


pie(df$value, labels = names(df), edges = 200, radius = 0.8,
...
if(!require(plotrix)) install.packages("plotrix", dependencies = T)
library("plotrix")
...
pie3D(df$value, labels = df$group, radius = 1.8,
...
plot(wt, 
     mpg, 
     main="Scatterplot Example",  xlab="Car Weight ",
     ylab="Miles Per Gallon ", pch=19, frame=FALSE)
...
smoother = FALSE, grid = FALSE, frame =
FALSE)
...
legend("topright", inset=.05, title="Number of
     Cylinders", c("4","6","8"), fill=terrain.colors(3), horiz=TRUE)
